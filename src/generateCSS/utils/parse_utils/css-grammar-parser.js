...
const primitives = new Map([
  ...
  [
    "custom-ident",
    { url: "https://drafts.csswg.org/css-values-4/#custom-idents" },
  ],
  ...
  [
    "number-percentage",
    { url: "https://drafts.csswg.org/css-values-4/#number-percentage" },
  ],
  [
    "length-percentage",
    { url: "https://drafts.csswg.org/css-values-4/#length-percentage" },
  ],
  [
    "frequency-percentage",
    { url: "https://drafts.csswg.org/css-values-4/#frequency-percentage" },
  ],
  [
    "angle-percentage",
    { url: "https://drafts.csswg.org/css-values-4/#angle-percentage" },
  ],
  [
    "time-percentage",
    { url: "https://drafts.csswg.org/css-values-4/#time-percentage" },
  ],
  ...
  [
    "position",
    { url: "https://drafts.csswg.org/css-values-4/#typedef-position" },
  ],
]);
...
const componentizeByCombinators = (
  parts,
  combinators = new Map(combinatorsMap)
) => {
  const res = {};
  ...
  while (!combinatorFound) {
    ...
    if (Array.isArray(parts) && parts.includes(c)) {
      ...
      res[t] = components.map((p) =>
        componentizeByCombinators(p, lowerCombinators)
      );
    }
  }
  if (!combinatorFound) {
    if (Array.isArray(parts)) {
      if (parts.length > 1) {
        return {
          type: "array",
          items: parts.map((p) => componentizeByCombinators(p)),
        };
      } else {
...
const parseMultiplierRange = (range) => {
  ...
  if (values.match(/^[0-9]+$/)) {
    ...
    return {
      ...{ minItems: parseInt(min, 10) },
      ...(max ? { maxItems: parseInt(max, 10) } : {}),
    };
  } else {
...
const isMultiplier = (s) =>
  typeof s === "string" &&
  multipliersStarters.map((starter) => s.startsWith(starter)).includes(true);

const primitiveMatch = (s, p) =>
...
const parseTerminals = (s) => {
  let m;
  if (
    [...new Map(combinatorsMap).keys()].includes(s) ||
    s === "[" ||
    s.startsWith("]") ||
    isMultiplier(s)
  ) {
    ...
    return Object.assign(
      { type: "primitive", name },
      range ? { range: parseBracketedRange(range) } : {}
    );
  } else if ((m = s.match(/^<[-_a-zA-Z]([^>]*)>$/))) {
    ...
    // TODO: add support for functional notations https://drafts.csswg.org/css-values-4/#functional-notation even though they're not recognized as top-level items in the grammar
  ...
  while (i < value.length) {
    ...
    if (c.match(/\s/)) {
      ...
      if (delimiterStates.includes(state)) {
        ...
        throw new Error(
          `Unexpected < in ${currentToken} while parsing ${value} in state ${state}`
        );
      }
    } else if (c === ">") {
      if (state === "quote") {
      ...
      if (delimiterStates.includes(state)) {
        ...
        throw new Error(
          `Unexpected ${c} in ${currentToken} while parsing ${value} in state ${state}`
        );
      }
    } else if (c === "{") {
      if (state === "quote") {
      ...
      if (state === "new" || state === "keyword") {
        // 'pipe' can't appear just before ampersand
        if (currentToken) tokens.push(currentToken);
        currentToken = c;
        state = "ampersand";
      } else if (state === "quote") {
...
export const parsePropDefValue = (value) => {
  ...
  while (parts.findIndex((p) => p.type === "functionstart") !== -1) {
    ...
    const matchingClosingFuncIdx = parts.findIndex(
      (p, i) => p.content === ")" && i > funcIdx
    );
  ...
  while (parts.lastIndexOf("[") !== -1) {
    ...
    const matchingBracketIdx = parts.findIndex(
      (p, i) => p === "]" && i > bracketIdx
    );
    ...
    let multiplier,
      ...
      multiplied = false;
    while (
      (multiplier = parts.slice(matchingBracketIdx + 1)[i]) &&
